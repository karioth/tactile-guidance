...
class EfficientTAMVideoPredictor(EfficientTAMBase):
    ...
    def init_state(
        ...
        if video_path is not None:
            ...
            # Real-time streaming mode (like SAM2)
    ...
    def add_new_frame(self, inference_state, new_image):
        ...
        This method provides SAM2 compatibility for streaming mode.
        ...
        if isinstance(new_image, torch.Tensor):
            img_tensor = new_image
        else:
            # Convert from numpy (HWC) to tensor (CHW)
            if len(new_image.shape) == 3 and new_image.shape[2] == 3:
                img_tensor = torch.from_numpy(new_image).permute(2, 0, 1).float()
            else:
                img_tensor = torch.from_numpy(new_image).float()
        
        # Ensure tensor is on the right device
        img_tensor = img_tensor.to(device)
        
        # Resize and normalize the image to match model's expected input
    ...
    def infer_single_frame(self, inference_state, frame_idx):
        ...
        This method provides SAM2 compatibility.
        ...
        if batch_size == 0:
            # No objects to track
            return frame_idx, obj_ids, torch.empty(0, 1, inference_state["video_height"], inference_state["video_width"])
        
        # Check if we need to consolidate temporary outputs first
        # This is needed when add_new_points_or_box was called but propagate_in_video_preflight wasn't
        needs_preflight = False
        for obj_idx in range(batch_size):
            temp_output_dict = inference_state["temp_output_dict_per_obj"][obj_idx]
            if (len(temp_output_dict["cond_frame_outputs"]) > 0 or 
                len(temp_output_dict["non_cond_frame_outputs"]) > 0):
                needs_preflight = True
                break
        
        if needs_preflight:
            # Consolidate temporary outputs before inference
            self.propagate_in_video_preflight(inference_state)
        ...
        for obj_idx in range(batch_size):
            ...
            # Check if we already have output for this frame
            if frame_idx in obj_output_dict["cond_frame_outputs"]:
        ...
        # Convert to original video resolution
        _, video_res_masks = self._get_orig_video_res_output(
            inference_state, all_pred_masks
        )
